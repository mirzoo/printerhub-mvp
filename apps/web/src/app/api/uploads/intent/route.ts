import { deviceIdSchema, MAX_FILE_SIZE } from "@printerhub/contracts";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = z.object({ deviceId: deviceIdSchema, size: z.number().int().positive().max(MAX_FILE_SIZE) }).safeParse(await safeJson(request));
