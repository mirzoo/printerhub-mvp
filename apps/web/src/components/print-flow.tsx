...
import { MAX_COPIES, MAX_FILE_SIZE, MAX_PAGES } from "@printerhub/contracts";
...
export function PrintFlow({ deviceId }: { deviceId: string }) {
  ...
  async function unlock(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        const message = typeof result?.message === "string" ? result.message : "Не удалось проверить ПИН‑код. Попробуйте снова";
  ...
  async function submit() {
    ...
    try {
      const intentResponse = await fetch("/api/uploads/intent", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ deviceId, size: file.size }),
      });
      ...
      const jobResponse = await fetch("/api/jobs", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ deviceId, pathname: intent.pathname, copies }),
      });
    ...
    return (
      <main className="shell compact-shell">
        ...
        <section className="card empty-state">
          ...
          <h1 className="state-title">Аппарат не найден</h1>
  ...
  return (
    <main className="shell">
      ...
      <p className="lead">Загрузите PDF с телефона — аппарат выполнит задание автоматически</p>
