...
На экране аппарата также доступно «Копирование». Пользователь кладёт лист на стекло, запускает сканирование на kiosk‑экране, проверяет страницы и проходит тот же checkout. Скрытый Mac Agent управляет сканером через ImageCaptureCore; клиент не получает доступ к macOS, USB или CUPS.
...
- Для копирования: совместимый ICA scanner driver и Apple Command Line Tools со Swift compiler
...
Один раз соберите нативный scanner helper на Mac:

```bash
npm run build:scanner -w @printerhub/agent
```

Проверьте сканер в «Системных настройках» → «Принтеры и сканеры» → «Открыть сканер». Если подключено несколько сканеров, укажите часть точного имени в `SCANNER_NAME`.

## Копирование

Kiosk‑страница обращается к Agent только через `127.0.0.1:17654`. Loopback bridge принимает только origin из `API_BASE_URL`, поэтому публично открытый kiosk URL не может удалённо запустить физический сканер.

Каждый лист сканируется отдельно: A4, 300 DPI, градации серого. Можно проверить предпросмотр, пересканировать или удалить страницу и добавить до 20 листов. После checkout backend объединяет одностраничные PDF и создаёт обычное задание печати. Предпросмотры и исходные страницы удаляются после оплаты, отмены или истечения 10‑минутной сессии.
...
Миграция применяет базовую схему, заказы и таблицы временных сессий копирования. Имена пользовательских файлов в БД не сохраняются.
...
- `NEXT_PUBLIC_KIOSK_LOOPBACK_PORT` — локальный порт bridge, по умолчанию `17654`
...
- `SCANNER_HELPER_PATH` — путь к собранному `printerhub-scan`
- `SCANNER_NAME` — необязательная часть имени сканера; обязательна, если подключено несколько устройств
- `KIOSK_LOOPBACK_PORT` — локальный порт bridge, по умолчанию `17654`

Web и Agent должны использовать одинаковый loopback port.
...
Проверка production выполняется в `dry-run`: открыть URL с телефона, ввести PIN, загрузить тестовый PDF, дождаться четырёх статусов и убедиться, что Blob удалён. `real` smoke test выполняется только когда очередь Brother видна в `lpstat`, а scanner helper находит устройство. Для копирования отсканируйте 2 листа, пересканируйте один из них, откройте checkout, подтвердите оплату и проверьте физическую печать и очистку приватных файлов.
